...
template<typename T>
class VirusFactory : public GameObject {
    public:
        using iterator = std::vector<T>;
        VirusFactory(GLfloat x, GLfloat y, GLfloat z) :
        GameObject(x,y,z), m_spawn_interval(GameVariables::rand_spawn_interval()), 
        m_update_tick(GameVariables::TICKS_PER_SEC * m_spawn_interval),
        m_remaning_spawn_ticks(m_update_tick), 
        m_active_interval(GameVariables::rand_active_interval() * GameVariables::TICKS_PER_SEC),
        m_remaning_active(m_active_interval), 
        m_sleep_interval(GameVariables::rand_sleep_interval() * GameVariables::TICKS_PER_SEC),
